...
// 
//
// Related Topics Array Depth-First Search Breadth-First Search Matrix 👍 4903 ?
//? 478
...
class Solution {
    public int[][] floodFill(int[][] image, int sr, int sc, int color) {
        ...
        int left = sc - 1;
        int right = sc + 1;
        if (up >= 0 && image[up][sc] == cur) floodFill(image, up, sc, color);
        if (down < image.length && image[down][sc] == cur) floodFill(image, down, sc, color);
        if (left >= 0 && image[sr][left] == cur) floodFill(image, sr, left, color);
        if (right < image[0].length && image[sr][right] == cur) floodFill(image, sr, right, color);
        return image;
    }
}
