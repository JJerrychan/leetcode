...
// 
//
// Related Topics Array Heap (Priority Queue) Simulation 👍 230 👎 207
...
class Solution {
    public int getNumberOfBacklogOrders(int[][] orders) {
        ...
        PriorityQueue<int[]> buyQueue = new PriorityQueue<>((a, b) -> {
            return b[0] - a[0];
        });
        PriorityQueue<int[]> sellQueue = new PriorityQueue<>((a, b) -> {
            return a[0] - b[0];
        });
        for (int[] order : orders) {
            int price = order[0], amt = order[1], type = order[2];
            if (type == 0) {
                while (amt > 0 && !sellQueue.isEmpty() && sellQueue.peek()[0] <= price) {
                    int[] sellOrder = sellQueue.poll();
                    int sellNum = Math.min(sellOrder[1], amt);
                    amt -= sellNum;
                    sellOrder[1] -= sellNum;
                    if (sellOrder[1] > 0) sellQueue.offer(new int[]{sellOrder[0], sellOrder[1]});
                }
                if (amt > 0) buyQueue.offer(new int[]{price, amt});
            } else {
                while (amt > 0 && !buyQueue.isEmpty() && buyQueue.peek()[0] >= price) {
                    int[] buyOrder = buyQueue.poll();
                    int sellNum = Math.min(amt, buyOrder[1]);
                    amt -= sellNum;
                    buyOrder[1] -= sellNum;
                    if (buyOrder[1] > 0) buyQueue.offer(new int[]{buyOrder[0], buyOrder[1]});
                }
                if (amt > 0) sellQueue.offer(new int[]{price, amt});
            }
        }
