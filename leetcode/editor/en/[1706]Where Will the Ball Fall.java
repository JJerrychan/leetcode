...
class Solution {
    public int[] findBall(int[][] grid) {
        ...
        int[] balls = new int[col];
        row--;
        col--;
        for (int ball = 0; ball <= col; ball++) {
            int currR = 0, currC = ball;
            while (currR <= row) {
                if (currC == 0 && grid[currR][currC] == -1) {
                    balls[ball] = -1;
                    break;
                }
                if (currC == col && grid[currR][currC] == 1) {
                    balls[ball] = -1;
                    break;
                }
                if (grid[currR][currC] == 1 && grid[currR][currC + 1] == -1) {
                    balls[ball] = -1;
                    break;
                }
                if (grid[currR][currC] == -1 && grid[currR][currC - 1] == 1) {
                ...
                if (grid[currR][currC] == 1) currC++;
                else currC--;
                currR++;
            }
            balls[ball] = balls[ball] == -1 ? -1 : currC;
        }
        return balls;
    }
}
